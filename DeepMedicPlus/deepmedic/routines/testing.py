...
def prepare_feeds_dict(feeds, channs_of_tiles_per_path):
    ...
    for path_i in range(len(channs_of_tiles_per_path[2:])):
        feeds_dict.update(
            {feeds['x_sub_' + str(path_i)]: np.asarray(channs_of_tiles_per_path[2 + path_i], dtype='float32')})

    return feeds_dict
    

def predict_whole_volume_by_tiling(log, sessionTf, cnn3d,
                                   channels, prior_channels, roi_mask, inp_shapes_per_path, unpred_margin,
...
def inference_on_whole_volumes(sessionTf,
    ...
    for subj_i in range(n_subjects):
        ...
        pad_left_right_per_axis) = preproc_imgs_of_subj(log, "",
                                                        channels, prior_channels, gt_lbl_img, roi_mask, None,
