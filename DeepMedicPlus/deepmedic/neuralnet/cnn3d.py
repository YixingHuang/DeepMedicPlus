...
class Cnn3d(object):
    ...
    def setup_ops_n_feeds_to_train(self, log, inp_plchldrs, p_y_given_x, total_cost, updates_of_params_wrt_total_cost) :
        ...
        self._feeds_main['train']['x_prior'] = inp_plchldrs['x_prior']
    ...
    def setup_ops_n_feeds_to_val(self, log, inp_plchldrs, p_y_given_x):
        ...
        self._feeds_main['val']['x_prior'] = inp_plchldrs['x_prior']
        for subpath_i in range(self.numSubsPaths) : # if there are subsampled paths...
    ...
    def setup_ops_n_feeds_to_test(self, log, inp_plchldrs, p_y_given_x, indices_fms_per_pathtype_per_layer_to_save=None) :
        ...
        self._feeds_main['test']['x_prior'] = inp_plchldrs['x_prior']
        for subpath_i in range(self.numSubsPaths) : # if there are subsampled paths...
    ...
    def _setup_inp_plchldrs(self, train_val_test, inp_shapes_per_path): # TODO: REMOVE for eager
        ...
        inp_plchldrs['x_prior'] = tf.compat.v1.placeholder(dtype="float32",
                                                    shape=[None, self.pathways[0].get_n_fms_in()] + inp_shapes_per_path[
                                                        0], name='inp_x_prior_' + train_val_test)
        for subpath_i in range(self.numSubsPaths): # if there are subsampled paths...
            inp_plchldrs['x_sub_'+str(subpath_i)] = tf.compat.v1.placeholder(dtype="float32", shape=[None, self.pathways[0].get_n_fms_in()]+inp_shapes_per_path[subpath_i+2], name="inp_x_sub_"+str(subpath_i)+'_' + train_val_test)
        return inp_plchldrs
    ...
    def make_cnn_model( self,
        ...
        # =======================Make the Second NORMAL PATHWAY of the CNN, which is for prior data=======================
        thisPathway = NormalPathway()
        self.pathways.append(thisPathway)
        thisPathwayType = thisPathway.pType()
        thisPathWayNKerns = nkerns
    ...
    def apply(self, inputs_per_pathw, mode, train_val_test, verbose=True, log=None):
        ...
        # ===== Apply High-Res prior path =========
        input = inputs_per_pathw['x_prior']
        out = self.pathways[1].apply(input, mode, train_val_test, verbose, log)
        # dims_outp_pathway_hr_prior = out.shape
        fms_from_paths_to_concat.append(out)
        
        # === Subsampled pathways =========
        for subpath_i in range(self.numSubsPaths):
            input = inputs_per_pathw['x_sub_'+str(subpath_i)]
            this_pathway = self.pathways[subpath_i+2]
