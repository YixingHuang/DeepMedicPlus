...
class TrainSessionParameters(object):
    ...
    def __init__(self,
        ...
        if cfg[cfg.DATAFRAME_TR] is not None:
            ...
            if cfg[cfg.CHANNELS_TR] is None:
                self.errReqChansTr()
            if cfg[cfg.PRIOR_CHANNELS_TR] is None:
                self.errReqChansTr()
            ...
            self.prior_channels_fpaths_tr = parse_fpaths_of_channs_from_filelists(cfg[cfg.PRIOR_CHANNELS_TR], abs_path_cfg)
        ...
        if (not self.val_on_samples_during_train) and (not self.val_on_whole_volumes):
            ...
            self.prior_channels_fpaths_val = []
            ...
            if cfg[cfg.PRIOR_CHANNELS_VAL]:
                self.prior_channels_fpaths_val = parse_fpaths_of_channs_from_filelists(cfg[cfg.PRIOR_CHANNELS_VAL],
                                                                                     abs_path_cfg)
            else:
                self.errReqChannsVal()
    ...
    def print_params(self):
        ...
        logPrint("Filepaths to Channels of Validation Cases = " + str(self.prior_channels_fpaths_val))
