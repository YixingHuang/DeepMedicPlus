...
def get_paths_from_df(log, df, abs_path, req_gt=True):
    ...
    p_names = sorted([c for c in list(df.columns) if c.startswith('priorChannel_')])
    if len(p_names) == 0:
        # no channels error raise - move to function later
        log.print3('No prior channel columns in dataframe. Columns should be named "prior_channel_[channel_name]". Exiting')
        exit(1)
    # [[case1-ch1, case1-ch2], ..., [caseN-ch1, caseN-ch2]]
    prior_channels = [[normfullpath(abs_path, c) for c in list(item[p_names])] for _, item in df.iterrows()]
    ...
    return channels, prior_channels, gt, roi, pred
